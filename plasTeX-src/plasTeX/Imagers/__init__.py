...
class Dimension(float):
    ...
    Check dvipng.py command -D value
    fontSize depends on the resolution requested from dvipng.py
    and the pixel width of the reader display
    """
    # Adjusted according to dvipng.py settings
    fontSize = 26.0 # to convert px to em.
    @property
    def ex(self): 
...
class Image(object):
    ...
    # EJB: height of the image in the units given by the method in Dimension.method
    # EJB: note class Dimension is not instantiated but used directly with @property
    def height():
        def fget(self):
        ...
        def fset(self, value):
            if value is None:
                ...
                self._height = Dimension(value) # EJB
        return locals()
    height = property(**height())

    # EJB: width of the image
    def width():
        def fget(self):
    ...
    # EJB: vertical-align of the image
    def depth():
        def fget(self):
        ...
        def fset(self, value):
            if value is None:
                ...
                self._depth = Dimension(value) # EJB
        return locals()
